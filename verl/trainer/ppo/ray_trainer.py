...
LANGUAGE_REASONING_MAP = {
    ...
    "ID": "\nSilakan bernalar selangkah demi selangkah, dan letakkan jawaban akhir Anda di dalam \\boxed{}."
}
...
class RayStSTrainer(RayPPOTrainer):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    for k in sample_accs.keys():
                        ...
                        # 准确率>=下限的数据都可以进入后续阶段
                        if sample_accs[k] >= self.translation_acc_lower:
                            ...
                            if self.translation_acc_lower <= sample_accs[k] < self.translation_acc_upper:
                    ...
                    for eq in english_questions:
                        ...
                        translation_input = self.translation_prompt.format(
                            language=LANGUAGE_MAP[self.target_language],
                        ...
                        translation_input += f"<think>\nOkay, I will translate the English question into {LANGUAGE_MAP[self.target_language]}."
                    ...
                    for idx, response in enumerate(responses_strs):
                        try:
                            # 提取翻译结果
                            extracted = extract_last_translation(response, self.target_language)
                    ...
                    for mq in multilingual_questions:
                        reasoning_input = mq + LANGUAGE_REASONING_MAP[self.target_language]
                        ...
                        reasoning_input += LANGUAGE_START_PREFIX_MAP[self.target_language]
...
class RayStSSoloLanguageTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
...
class RayStSSoloLanguageOnlyStageOneTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    for k in sample_accs.keys():
                        ...
                        if sample_accs[k] > 0 and sample_accs[k] < self.translation_acc_upper:
                            stage1_training_indices.extend(cur_uid_indices)
                    ...
                    print(f">>> 最终训练数据组成: 阶段一: {len(stage1_final_batch)}, 总计: {len(batch)}")
                    print_sts_metrics = {k.replace('StS-Valid-Ratio/', ''): v for k, v in sts_metrics.items()}
                    print(f">>> StS数据有效比例: {print_sts_metrics}")
...
class RayStSSoloLanguageFilterTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
...
class RayStSSoloLanguageFilterTrainer0114(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第一阶段把大于0的部分全部纳入训练
    def fit(self):
        """
...
class RayStSSoloLanguageFilterTrainer0114Comet(RayPPOTrainer):
    ...
    def fit(self):
        ...
        # self.comet_server_url = self.config.data.get("comet_server_url", "http://localhost:8002")
        self.comet_server_url = os.getenv("COMET_SERVER_URL", "http://localhost:8002")
        self.comet_score_min = 0.05  # Comet分数最小值下限
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 使用Comet为翻译打分
                    print(f">>> 使用Comet为翻译打分")
                    with marked_timer("stage2_comet_scoring", timing_raw, color="yellow"):
                        import requests
                        comet_scores = {}
                        comet_requests = []
                        
                        # 准备Comet评分请求数据
                        # 由于stage2_batch经过了repeat，需要从原始问题中获取英文源文本
                        translation_sample_n = self.config.actor_rollout_ref.rollout.translation_sample_n
                        original_question_count = len(stage2_batch) // translation_sample_n
                        
                        # 记录索引计算相关信息
                        print(f">>> Comet评分准备: translation_sample_n={translation_sample_n}, original_question_count={original_question_count}, stage2_batch长度={len(stage2_batch)}, valid_indices数量={len(valid_indices)}")
                        if "en_question" in stage2_batch.non_tensor_batch:
                        ...
                        for idx in valid_indices:
                            # 获取对应的原始问题索引
                            # interleave=True的情况：每组连续translation_sample_n个样本属于同一原始问题
                            # 例如 translation_sample_n=2: [Q0,Q0, Q1,Q1, Q2,Q2] → idx=0,1→orig_q_idx=0; idx=2,3→orig_q_idx=1
                            if translation_sample_n > 1:
                                orig_q_idx = idx // translation_sample_n
                            else:
                                orig_q_idx = idx
                            
                            # 从en_question中获取原始英文问题
                            # en_question也被interleave repeat了，所以en_question[idx]就是第idx个样本对应的原始问题
                            if "en_question" in stage2_batch.non_tensor_batch:
                            ...
                            # 记录前几个样本的详细信息用于调试
                            if idx < min(5, len(valid_indices)):
                                src_preview = src_text[:50] + "..." if len(src_text) > 50 else src_text
                                mt_preview = mt_text[:50] + "..." if len(mt_text) > 50 else mt_text
                                print(f">>> Comet样本[{idx}]: orig_q_idx={orig_q_idx}, src={src_preview}, mt={mt_preview}")
                            
                            comet_requests.append({
                                "src": src_text,
                                "mt": mt_text
                            })
                        ...
                        # 批量调用Comet服务
                        try:
                            comet_url = f"{self.comet_server_url}/comet_score_batch"
                            response = requests.post(comet_url, json=comet_requests, timeout=300)
                            ...
                            if response.status_code == 200:
                                ...
                                for i, idx in enumerate(valid_indices):
                                    if i < len(scores):
                                        comet_scores[idx] = scores[i]
                                    else:
                                        comet_scores[idx] = self.comet_score_min
                                
                                print(f">>> Comet评分完成: {len(scores)}个翻译已评分")
                                print(f">>> Comet分数范围: min={min(scores):.4f}, max={max(scores):.4f}, mean={np.mean(scores):.4f}")
                                
                                # 记录Comet分数统计
                                metrics['sts/stage2_comet_score_mean'] = np.mean(scores).item()
                                metrics['sts/stage2_comet_score_std'] = np.std(scores).item()
                                metrics['sts/stage2_comet_score_max'] = np.max(scores).item()
                                metrics['sts/stage2_comet_score_min'] = np.min(scores).item()
                            else:
                                print(f">>> Comet服务调用失败: HTTP {response.status_code}, {response.text}")
                                # 失败时所有翻译分数设为最小值下限
                                for idx in valid_indices:
                                    comet_scores[idx] = self.comet_score_min
                        except Exception as e:
                            print(f">>> Comet服务调用异常: {e}")
                            # 异常时所有翻译分数设为最小值下限
                            for idx in valid_indices:
                                comet_scores[idx] = self.comet_score_min
                    ...
                    # 使用Comet分数来更新翻译奖励
                    # 直接使用Comet分数作为奖励值，但设置最小值下限
                    # 如果Comet分数 < 最小值下限，使用最小值下限；否则直接使用Comet分数
                    for idx in valid_indices:
                        comet_score = comet_scores.get(idx, 0.0)
                        # 应用最小值下限
                        reward_value = max(comet_score, self.comet_score_min)
                    ...
                    # 注意：翻译奖励已经在第二阶段通过Comet分数确定了，不再使用第三阶段的推理准确率来更新翻译奖励
                    # 第三阶段仍然执行，用于多语言推理训练
                    stage3_final_indices = []  # 只保留多语言推理准确率>0的数据
                    ...
                    if DEBUGGING_STS:
                        ...
                        print(f"失败翻译: {len(stage2_final_indices) - len(valid_indices)} 个 ({(len(stage2_final_indices) - len(valid_indices))*100/len(stage2_batch):.2f}%) [奖励保持0]")
                        print(f"stage3_final_indices数量: {len(stage3_final_indices)} (占stage3_batch的{len(stage3_final_indices)*100/len(stage3_batch):.2f}%)")
                        print(f"stage2_batch中非零奖励数量: {(stage2_batch.batch['reward_tensor'].sum(dim=1) > 0).sum().item()} [由Comet分数确定]")
                        print(f"stage2_batch中零奖励数量: {(stage2_batch.batch['reward_tensor'].sum(dim=1) == 0).sum().item()} [翻译失败]")
                        print(f"Comet分数最小值下限: {self.comet_score_min}")
                        # 计算奖励统计
                        non_zero_rewards = stage2_batch.batch['reward_tensor'].sum(dim=1)[stage2_batch.batch['reward_tensor'].sum(dim=1) > 0]
                        if len(non_zero_rewards) > 0:
                            print(f"奖励值范围: min={non_zero_rewards.min().item():.4f}, max={non_zero_rewards.max().item():.4f}, mean={non_zero_rewards.mean().item():.4f}")
                        print(f"=========================================\n")
...
class RayStSSoloLanguageFilterTrainer0114LLMJudge(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第一阶段把大于0的部分全部纳入训练
    def fit(self):
        """
...
class RayStSSoloLanguageFilterTrainer0210(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第一阶段把大于0的部分全部纳入训练
    def fit(self):
        """
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # OOM优化1: 阶段一generate结束后立即释放gen_batch_output（不参与计算图，可安全删除）
                    import torch
                    ...
                    # OOM优化4: stage1的batch在进入阶段二之前提前裁剪，减少阶段二三期间的显存占用
                    # 先提取stage1的训练数据（如果后续还需要）
                    stage1_training_batch_for_later = None
                    if len(stage1_training_indices) > 0:
                        # 注意：这里先不创建stage1_final_batch，等最后再创建，避免重复占用显存
                        pass
                    ...
                    # OOM优化4: 阶段二开始后，原始的batch已经不需要了（除了stage1_training_indices）
                    # 但为了安全，我们保留batch直到阶段一数据真正不需要为止
                    # 注意：如果stage1_training_indices为空，可以在这里释放batch
                    if len(stage1_training_indices) == 0:
                        import torch
                        del batch
                        if torch.cuda.is_available():
                    ...
                    with marked_timer("stage2_translation", timing_raw, color="blue"):
                        ...
                        # OOM优化1: 阶段二generate结束后立即释放中间变量
                        import torch
                        del trans_gen_batch_padded, trans_output_padded, trans_gen_batch
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                    ...
                    # OOM优化2: 阶段三generate之前，尝试将stage2_batch中不需要的tensor移到CPU
                    # 这是显存峰值最高的时刻，此时stage1的batch、stage2_batch、stage3的generate都同时占用显存
                    # 注意：这个优化需要谨慎，因为stage2_batch在阶段三结束后还需要用于更新奖励
                    # 如果DataProto支持.to()方法，可以尝试：stage2_batch.to("cpu")，generate完再移回GPU
                    # 如果不支持，可以手动将部分tensor移到CPU（但需要确保不影响后续使用）
                    ...
                    with marked_timer("stage3_multilingual_reasoning", timing_raw, color="green"):
                        ...
                        # OOM优化1: 阶段三generate结束后立即释放中间变量
                        import torch
                        del multi_gen_batch_padded, multi_output_padded, multi_gen_batch
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                    ...
                    # OOM优化3: 在concat前保存长度信息，concat后立即释放中间batch
                    stage1_len = len(stage1_final_batch) if isinstance(stage1_final_batch, DataProto) else 0
                    stage2_len = len(stage2_final_batch) if isinstance(stage2_final_batch, DataProto) else 0
                    stage3_len = len(stage3_final_batch) if isinstance(stage3_final_batch, DataProto) else 0
                    
                    batch = DataProto.concat(final_batches)
                    
                    # OOM优化3: concat后立即释放中间batch，减少compute_log_prob时的显存占用
                    import torch
                    del stage1_final_batch, stage2_final_batch, stage3_final_batch, final_batches
                    # 如果stage2_batch和stage3_batch还在内存中，也释放掉
                    if 'stage2_batch' in locals():
                        del stage2_batch
                    if 'stage3_batch' in locals():
                        del stage3_batch
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    
                    print(f">>> 最终训练数据组成: 阶段一: {stage1_len} | 阶段二: {stage2_len} | 阶段三: {stage3_len} | 总计: {len(batch)}")
                    print_sts_metrics = {k.replace('StS-Valid-Ratio/', ''): v for k, v in sts_metrics.items()}
                    print(f">>> StS数据有效比例: {print_sts_metrics}")
                    
                    # 添加数据统计（使用之前保存的长度信息，因为变量已被删除）
                    sts_metrics['StS-Valid-Ratio/Num-Stage1'] = stage1_len
                    sts_metrics['StS-Valid-Ratio/Num-Stage2'] = stage2_len
                    sts_metrics['StS-Valid-Ratio/Num-Stage3'] = stage3_len
                    sts_metrics['StS-Valid-Ratio/Num-Total'] = len(batch)
                    metrics.update(sts_metrics)
...
class RayStSSoloLanguageFilterTrainer0128(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第一阶段把大于0的部分全部纳入训练
    # thinking使用英文进行思考
    def fit(self):
...
class RayStSSoloLanguageFilterTrainerRepair(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第二阶段去除前缀
...
class RayStSSoloLanguageFilterTrainerRepair0114(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
    # 第二阶段去除前缀
    # 第一阶段把大于0的部分全部纳入训练
...
class RayStSOtherTranslationSoloLanguageFilterTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    for eq, lang in zip(english_questions, stage2_languages):
                        ...
                        translation_input = self.translation_prompt.format(
                            language=LANGUAGE_MAP[lang.upper()],
                            question=clean_question
                        )
                        translation_inputs.append(translation_input)
                    translation_sample_n = self.config.actor_rollout_ref.rollout.translation_sample_n
                    translation_inputs_base = []
                    for i in range(len(translation_inputs)):
                        for _ in range(translation_sample_n):
                            translation_inputs_base.append(translation_inputs[i])
                    translation_inputs = translation_inputs_base
                    
                    # 提取翻译结果
                    valid_translation_indices = []
                    translated_questions = []
                    responses_strs = run_async_translation(
                        texts=translation_inputs,
                        api_base_url=os.environ.get("API_BASE_URL", None),
                        api_key=os.environ.get("API_KEY", None),
                        model_name=os.environ.get("TRANSLATION_MODEL_NAME", None),
                        max_concurrency=20,
                    )
                    stage2_batch = stage2_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.translation_sample_n, interleave=True)
                    stage2_languages = stage2_batch.non_tensor_batch["sample_lang"].tolist()
...
class RayStSSoloLanguageFilterPaddingTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    ori_batch_length = len(batch)
                    world_size = self.actor_rollout_wg.world_size
                    remainder = ori_batch_length % world_size
                    if remainder != 0:
                        # 计算需要填充多少条数据才能凑齐
                        padding_size = world_size - remainder
                        
                        # 从现有数据中随机采样进行填充 (允许重复采样 replace=True)
                        # 这样既保证了不丢弃任何原始数据，又满足了分布式训练的形状要求
                        padding_indices = np.random.choice(ori_batch_length, padding_size, replace=True).tolist()
                        
                        # 选取填充数据
                        padding_batch = batch.select_idxs(padding_indices)
                        
                        # 将填充数据拼接到原始batch后面
                        batch = DataProto.concat([batch, padding_batch])
                        
                        print(f">>> 为保证能被world_size({world_size})整除，进行了Padding操作：")
                        print(f"    原始大小: {ori_batch_length}, 填充数量: {padding_size}, 最终大小: {len(batch)}")
                    else:
                        print(f">>> 数据大小({ori_batch_length})正好能被world_size({world_size})整除，无需Padding。")
                    # print(f">>> 为保证能被world_size整除，数据从{ori_batch_length}裁剪到{len(batch)}")
...
class RayStSStageOneENTrainer(RayPPOTrainer):
    """Self-improving via Self-translation Trainer"""
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    stage3_batch_keys_to_pop = ['prompts', 'responses', 'input_ids', 'attention_mask', 'position_ids']
                    stage3_non_tensor_batch_keys_to_pop = ["tools_kwargs"]
                    # # 注意：stage3_batch来自stage2_batch.select_idxs()，不包含timing等meta_info
                    # stage3_batch.pop(batch_keys=stage3_batch_keys_to_pop, non_tensor_batch_keys=stage3_non_tensor_batch_keys_to_pop)
                    meta_info_keys_to_pop = ["timing"]
                    stage3_batch.pop(batch_keys=stage3_batch_keys_to_pop, 
                        non_tensor_batch_keys=stage3_non_tensor_batch_keys_to_pop, 
