...
def compute_reward(data: DataProto, reward_fn):
    ...
    try:
        ...
        # reward_fn可能返回 (score_values, result_dict) 或 result_dict
        if isinstance(reward_result, tuple):
            # 如果返回的是tuple，那么第二个元素是dict
            score_values, result_dict = reward_result
            reward_tensor = result_dict["reward_tensor"]
            reward_extra_infos_dict = result_dict.get("reward_extra_info", {})
        else:
            # 如果直接返回dict
        ...
        # 处理旧格式返回值
        if isinstance(reward_result, tuple):
            reward_tensor = reward_result[1] if len(reward_result) > 1 else reward_result[0]
        else:
