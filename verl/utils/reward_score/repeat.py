...
# 1. 志鹏刚刚说的关键词的过滤["limited time", "time is limited", "time constraint", "时间有限", "时间限制", "i guess the answer", "我猜答案"]
# 2. 中文夹杂英文 codeswitch检测
# 3. repetition检测


if __name__ == "__main__":
    from utils import read_jsonl, save_to_jsonl
    # # examples = read_jsonl("/cpfs01/data/shared/Group-m6/lvchenxu.lcx/datasets/sft/development/v20250408/stage3/original/chat/math/en")
    examples = read_jsonl("/cpfs01/data/shared/Group-m6/jingjiu.zj/data/Qwen3_stage2_QWQ_tagged/qwen3_32b_sampling_liusx_stage3/math_processed_final3/exp4/zh/data_01_all1_4w_zh_queries_upsampled_twice.jsonl")
    # examples = read_jsonl("/cpfs01/data/shared/Group-m6/lvchenxu.lcx/datasets/sft/development/v20250408/stage3/original/long_thoughts/math/exp4/en/data_01_all1_4w_en_queries_upsampled_twice.jsonl")
    cnt1, cnt2, cnt3 = 0, 0, 0
    for e in tqdm(examples):
        response = e['messages'][-1]['content']
        is_response_valid = check_response_valid(response)
        is_codeswitch_valid = check_codeswitch_valid(response)
        is_repetition_valid = check_repetition_valid(response)
        if is_response_valid==False:
            cnt1 += 1
        if is_codeswitch_valid == False:
            cnt2 += 1 
        if is_repetition_valid == False:
            cnt3 += 1
        # if is_response_valid==False or is_codeswitch_valid==False or is_repetition_valid==False:
        #     # print(e['messages'][-1]['content'])
        #     # print(is_response_valid)
        #     # print(is_codeswitch_valid)
        #     # print(is_repetition_valid)
        #     import pdb 
        #     pdb.set_trace()
    print(cnt1)
    print(cnt2)
...
'''
python -m fix_tools.fix_all_problemable_patterns_all
